	...
	namespace searches
	{
		...
		void PathSearch::load(TileMap* _tileMap) {
			...
			for (int i = 0; i < _tileMap->getRowCount(); i++) {
				...
				for (int j = 0; j < _tileMap->getColumnCount(); j++) {
					map[i].push_back(_tileMap->getTile(i, j));
				}
			}
			...
			for (int i = 0; i < _tileMap->getRowCount(); i++) {
				for (int j = 0; j < _tileMap->getColumnCount(); j++) {
					adjTiles[map[i][j]] = findAdj(map[i][j]);
				}
			}
				...
				if (curr->vertex == goal) {
					...
					for (int i = 0; i < adjTiles[currTile].size(); i++) {
						...
						if (visited.find(successorTile) != visited.end()) {
							...
							PlannerNode* successor = new PlannerNode(successorTile);
		...
		void PathSearch::shutdown() {
			...
			for (auto it = visited.begin(); it != visited.end(); ) {
				delete it->second;
				// Prevent duplicate pointer deletion
